...
export class Value {
  ...
  asInteger(): bigint {
    ...
    if (typeof this.data !== "bigint") {
      throw new Error(
        `AsInteger: not an integer value (got ${typeof this.data})`,
      );
    }
    return this.data;
  }
  ...
  asBoolean(): boolean {
    ...
    if (typeof this.data !== "boolean") {
      throw new Error(
        `AsBoolean: not a boolean value (got ${typeof this.data})`,
      );
    }
    return this.data;
  }
  ...
  asChildType(): ChildType {
    if (!(this.data instanceof ChildType))
      throw new Error("AsChildType: not a typed container");
    return this.data;
  }
  ...
  asInterpSegments(): InterpSegment[] {
    if (!Array.isArray(this.data))
      throw new Error("AsInterp: not an interp string");
...
export function newList(
  elems: Value[],
  opts?: { eval?: boolean; quoted?: boolean },
): Value {
...
export class ChildType {
  ...
  constructor(
    child: Value,
    elements: Value[] = [],
    entries: { key: string; value: Value }[] = [],
  ) {
    this.child = child;
...
export interface MoveInfo {
  to: string;
  /** Why the move was emitted — diagnostic only, mirroring Go's Reason. */
  reason?: string;
  /** Present on a CONDITIONAL move: see IfCont. */
  ifCont?: IfCont;
}

/**
 * IfCont is the continuation an `if` with a LIST condition carries.
 *
 * The condition cannot be evaluated before the branches are chosen — it
 * is a code body, and running it eagerly would run it in the wrong place
 * on the tape. So the word emits `Mark · condition · MoveIf`, the engine
 * runs the condition where it stands, and the move reads the condition's
 * RESULT to pick a branch. `else` is absent for the two-argument form,
 * where a false condition produces nothing.
 *
 * Mirrors core/go/value.go's IfCont.
 */
...
/** A move carrying an if-continuation. Mirrors Go NewMoveIf. */
export function newMoveIf(to: string, reason: string, ifCont: IfCont): Value {
  return new Value(TMove, { to, reason, ifCont } satisfies MoveInfo);
}

let markCounter = 0;

/**
 * A fresh mark id. Marks pair a move with its anchor, so the ids only
 * have to be unique within one run; the counter is process-wide as Go's
 * is, which also keeps them stable to read in a trace.
 */
export function nextMarkId(): string {
  markCounter++;
  return `_m${markCounter}`;
}
...
export function asReach(v: Value): ReachInfo | undefined {
  if (!isReach(v) || v.data === null || typeof v.data !== "object")
    return undefined;
...
export function asSugar(v: Value): SugarInfo | undefined {
  if (!isSugar(v) || v.data === null || typeof v.data !== "object")
    return undefined;
...
export function renderXmlTmplSrc(t: XmlTmpl): string {
  ...
  for (const c of t.children) {
    if ("lit" in c) b += c.lit;
      ...
      b += "${" + c.expr.map((x) => x.toString()).join(" ") + "}";
