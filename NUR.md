...
**Fence.** The two descriptors are ledgered by name in
...
**Status:** FIXED 2026-09-23 (the trailing value's re-step — the handoff
log's entry of that date), with one LOUD residue pinned pending. Recorded
...
declines any re-step-marked element) DECLINE. The residue: `(2 (mk 1)) 10
mul` — the check pass steps the marked carrier as data, `mul` collects it
with the 10, and the compiled program raises the no-match the interpreter
(22) never does — LOUD, pinned pending by `TestParenTrailingFnLoudPending`.
The original record follows.
...
by `TestParenTrailingFnAgrees`, `TestParenTrailingFnSoundCompileFailures`
and `TestParenTrailingFnLoudPending` (lang/go,
paren_trailing_forward_test.go), and the fold row by
