...
**Status:** FIXED 2026-09-26 (the sweep's last cells — the handoff log's
entry of that date). Recorded 2026-09-18.

**The fix, in one paragraph.** The runtime took the second of the two
routes named below: the four natives wrap a COMPILED CLOSURE too
(`wrapCompiledClosure` → `core.UsurpClosure` / `ForceStackClosure` /
`ForceForwardClosure` / `ForceArityClosure`). The wrapper reads the
closure's signatures through the compiled runtime's bridge
(`ClosureAsFnDef`) and replaces every Impl with its own re-dispatch, so
nothing of the bridge's run-bound invoker survives into it; what it stores
(`Wraps`) and re-dispatches is the closure VALUE itself, which the VM's
dynamic apply now unwraps to and applies natively (callDynamic /
callDynMethod), and which an island's interpreter bridges at the one
dispatch it meets it in. With the runtime answering, `recordGradualWrap`
no longer declines a typed Function carrier, so a factory's returned
closure at these slots compiles (the sweep's `force-arity` /
`forward-args` / `usurp` / `stack-args` × factory cells), and a def of the
wrapper carries the closure's shape (usurp reversed) so its read declines
a written argument the wrapper does not take, as it does for any produced
closure. Pinned by `TestModifierOverFactoryClosureCompiles` (lang/go,
`sweep_cells_s2b_test.go`), which carries both witnesses below.
...
**The fix, in one paragraph.** The operands were never transposed by a
slot rule: the check pass saw `m.up` as a dynamic(Any) carrier, and `emit`'s
handler — finding no Function and no kind name at position 0 — took the
AUTO form, which reads a leading operand as the OPTIONS map and the last as
the data. The compiled lane then ran `emitlang-auto` with the fn as opts;
the interpreter, reading the real member, ran the value form. `emit` (and
`mini` and `parse`, which raised a false "the kind must be a literal name"
check error over the same carrier) now treat a GRADUAL leading operand as
undecidable under analysis: the compile pass records the module's runtime
fn-dispatch native (`emitlang-fn-dispatch`, `minilang-fn-dispatch`,
`parselang-fn-dispatch`) over the macro's own operands, and that native
does at run time exactly what the macro does with the value it finds — the
value form for a fn (a compiled closure bridged), the word re-run for
anything else (a kind atom, an options map). Pinned by
`TestMacroGradualLeadDispatchesAtRunTime` (lang/go,
`sweep_cells_s2b_test.go`); the `sweepKnownMiscompiles` pin is retired.
...
```
def c true end (if c ([x:Integer] => [x]) ([x:Integer] => [0])) 5
  interpreted   [fn (Integer) 5]   the paren PLACES its value (PAREN-RESTEP-RULE)
  compiled      [5]                the residual's lead arm applied it over 5

def c true end (if c ([] => [42]) ([] => [2])) apply
  interpreted   42                 `apply` marks the 0-arg lambda and re-steps it
  compiled      [fn]               the apply over the branch result is lost
```

**Where it sits.** The residual's may-be-fn lead arm (NUR159's) applies a
branch result over the entry after it on the event's `mayBeFn` flag alone —
it asks no placement question, where the carrier and dynamic arms ask
`leadPlacedNotRead` / `placedNotReStepped`. The `apply` shape is the
record side: `apply` over a branch result the pass holds as a Function
carrier records as the identity it is for a concrete fn, and the landing
that would re-step the marked value is not emitted.
...
module-export cells — the handoff log's entry of that date). Found the
same day, probing the `behave` × container cell's neighbours; present on
`main` at 89dd499 (every witness measured before the fix).

**The witnesses.**

```
def Temp refine Integer end
def mk fn [[k:String][Function][(fn [[t:Temp][String][k]])]] end def k 'Z' end
behave canon/q (mk 'K') end canon (make Temp 5)
  interpreted   'Z'   (behave installs the source fn's body; `k` is read at canon time)
  compiled      behave canon: fn arg has invalid payload (core.ClosurePayload)

import "boru:fn-util"  def mk fn [[k:Integer][Function][(x:Integer => [add k x])]] end
def h (FnUtil.compose (mk 1) (mk 2)) end (h 5)
  interpreted   8
  compiled      FnUtil.compose: argument must be a function value, got fn (Integer)

def Temp refine Integer end def g fn [[][String] [def k 'K' canon (make Temp 5)]] end
def k 'Z' end behave canon/q (fn [[t:Temp][String][k]]) end g
  interpreted   'K'   (the behaviour runs inside g, where g's `k` shadows)
  compiled      'Z'
```

**Where it sat.** A CompileFnHandlerStrict slot's handler validates an
interpreter FnDefInfo — behave reads the fn's raw body tokens and declared
signature; fn-util reshapes it. A capturing `fn` / `=>` a factory returns is
an OpPushClosure on the compiled lane, and its bridged shape carries no
body and no return. RecordCallOperands declined only a CONCRETE capturing
fn at the slot; a CARRIER operand (the factory's declared `[Function]`
result) rode straight into the call. Separately, behave stores its body and
runs it LATER against the registry (userBehavior's RunPooledTop), resolving
names in the interpreter's dynamic scope, which the compiled lane mirrors
only under DynEnv — and behave's record never armed it.

**The fix.** The strict slot admits an operand only when the recorder
PROVES it arrives as an FnDefInfo (compiler `strictFnOperandProven`,
stored_fn_proof.go): a baked const fn, a monomorphic user call whose unit
returns one (a capture-free factory), or a member read over a const
...
`keep_defs_leak_test.go` (`TestDynamicKeepDefsBodyLeakDeclines`) and
compiler `kept_defs_test.go`. A run that may leave a callable seated ALONE
(or last) is still data on the compiled lane: NUR213.
...
def mk fn [[][List][quote [i]]] end 3 for (mk)
  interpreted   signature_error: cannot call `for`
  compiled      internal_error: DISPATCH_REMATCH at for matched at run time
                where the static model failed

def mk fn [[][List][quote [i]]] end (1 add 2) for (mk)
  (the same pair)
```

**Where it sits.** The forward `(mk)` fills `for`'s count slot, which no
overload accepts; the check pass's unmatched-dispatch recovery records a
rematch, and the rematch finds a match at run time the static model never
had, which the VM refuses as an internal error. Both lanes fail, with
different codes.
