...
4. **What the branch-carried def does not reach**, each declining where it
   ...
   NUR112 is CLOSED too (the plain check applies a stored fn member). NUR099 is CLOSED (a capitalised name over an undeclared fn body is refused with def_error; predicates are declared with `fnpred`, and the arity route is gone).
