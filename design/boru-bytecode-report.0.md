...
1. **Ahead-of-time.** A `boru compile` CLI produces a `.boruc`
...
17. **Bytecode ABI stability.** `.boruc` files baked across
...
19. **GOMAXPROCS / concurrent VMs.** If multiple goroutines run
    ...
    to regress.
20. **Tooling needs to understand two modes.** `boru repl`,
    `boru check`, `boru run` each need to know whether they
...
**Scenario.** A `.boruc` file is cached from a previous run. The
user edits `foo.boru` — adds a new signature to a user fn, or
...
2. **Default to eager compile at load, not persist.** Unless
   the user explicitly opts into `.boruc` files, compile in
...
in the `.boruc` header. Older CLI versions refuse forward-version
...
span tracing, coverage instrumentation, `.boruc` caching. Can
