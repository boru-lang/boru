...
`boru-bytecode-report.0.md` (the design) and
`boru-bytecode-revisions.0.md` (the June 2026 re-review that this plan
...
- **Two execution modes, one semantics.** The interpreter remains the
  default and the reference. Compiled mode is opt-in (`boru run
  --compile` / `BORU_COMPILE=1`) until the graduation criteria in
  Stage 7 are met.
...
3. *Default-on (Stage 7):* the flag inverts into a long-lived kill
   switch (`BORU_NO_COMPILE`), the `Registry.TCO.Disable` pattern.
...
that answer; surface them via `boru check --emit` (disassembly +
...
`boru-bytecode-baseline.0.md`. Dispatch machinery measured at ~96% of
...
Deliverable: `design/boru-bytecode-baseline.0.md` with the corpus, the
...
reason. Remaining Stage-1 items: the CLI surface (`boru check
...
- **Gate:** a `Program` disassembler (`boru check --emit` or similar,
...
- **Tooling/DX:** `BORU_COMPILE`/`BORU_NO_COMPILE` env (the rollout
  ...
  report (`boru check --emit` prints `sites: mono/poly/dynamic/meta` +
...
`design/boru-bytecode-runtime-independence.0.md`** — multi-result lowering,
...
3. Tooling parity for the supported surface: PC→span error rendering
   byte-identical, `boru check` diagnostics unchanged, trace mode has
...
`.boruc` persistence (compile in memory per run; staleness class
