...
## ADR-017 — Wire identity is never derived from the product name {#adr-017}

**Status:** Accepted · **Date:** 2026-08-24 · extends ADR-009

The product name is a codename and will change again. Identifiers this
project writes into places it does not own — magic bytes in files,
namespaces in OS credential stores — cannot change with it. Renaming one
does not rename that data; it makes it unreadable on machines we will
never see, and the failure presents as a correct passphrase being
rejected or a built executable no longer recognising itself.

This has happened twice. `AQL -> BORU` re-spelled the keyring magic, the
export magic and the executable trailer in one sweep. Because the new
spellings differed in **length** from the old, readers taking offsets
from `len(magic)` sliced salt, nonce and length fields from the wrong
bytes — silent, total data loss for every pre-rename vault, bundle and
built binary.

Three tiers, and only the last may follow the name:

- **Frozen** — on disk or in a credential store. Lives in
  `cmd/go/internal/wire`, brand-free, never deleted from the legacy read
  set. Readers take every offset from the magic that *matched*, never
  from the one they write.
- **Migratable** — user-visible surfaces (paths, env vars, the source
  extension, the import namespace). Change only with a dual-read window
  spanning at least one release.
- **Cosmetic** — text a human reads and nothing parses back. Lives in
  `cmd/go/internal/brand`.

The 2026-08 audit and `scripts/brand-audit.sh` classify every occurrence.

**Why tests did not catch it.** Every crypto test encrypted and decrypted
through the same constant it was meant to guard, so the sweep rewrote the
assertion alongside the code and the suite stayed green at 100% coverage
(ADR-008). A self-referential test cannot detect identifier drift. The
expectation must live outside the source: `cmd/go/internal/wire/testdata`
holds a binary fixture written under **every** identifier ever shipped,
and a find-and-replace cannot rewrite those bytes. Never regenerate a
fixture to make a test pass — a failure means shipped data just became
unreadable.

Changing a frozen identifier is therefore a four-part commit: move the old
value into the legacy read set, add its golden fixture, update the pins in
`wire_test.go`, and say why in this record.
