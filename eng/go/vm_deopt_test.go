...
func TestDeoptIfFnArms(t *testing.T) {
	...
	st, fired, err := vc.deoptIfFn(r, fn.Body, false, &spec, 0, []core.Value{five}, []core.Value{five}, seam7Dbg, 0)
	...
	// collects 5 → 6.
	st, fired, err = vc.deoptIfFn(r, fn.Body, false, &spec, 0, []core.Value{five}, []core.Value{inc}, seam7Dbg, 0)
	if err != nil || !fired || len(st) != 1 {
	...
	st, fired, err = vc.deoptIfFn(r, fn.Body, false, &depthSpec, 1, []core.Value{core.NewInteger(0), five, inc}, nil, seam7Dbg, 0)
	...
	_, _, err = vc.deoptIfFn(r, fn.Body, false, &spec, 0, nil, []core.Value{inc}, seam7Dbg, 0)
	...
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: 3}, 0, nil, []core.Value{inc}, seam7Dbg, 0); err == nil {
		t.Error("a slot beyond the frame raises")
	}
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: -1, Depth: 2}, 0, []core.Value{inc}, nil, seam7Dbg, 0); err == nil {
		t.Error("a depth below the frame base raises")
	}
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: 0, Depth: -1, Token: 9, RetPC: 1}, 0, nil, []core.Value{inc}, seam7Dbg, 0); err == nil {
...
func TestReStepIfFnArms(t *testing.T) {
	...
	st, fired, err := vc.deoptIfFn(r, fn.Body, false, &spec, 0, []core.Value{five, five}, nil, seam7Dbg, 0)
	if err != nil || fired || len(st) != 2 {
		t.Errorf("data: no-op, got %v %v %v", st, fired, err)
	}
	// A fn on top over the region [5], resuming at the body's end: the fn
	// re-steps as a token and collects the 5 beneath it → 6.
	st, fired, err = vc.deoptIfFn(r, fn.Body, false, &spec, 0, []core.Value{five, inc}, nil, seam7Dbg, 0)
	if err != nil || !fired || len(st) != 1 {
		t.Fatalf("fn: the island's residual replaces the region, got %v %v %v", st, fired, err)
	}
	if n, _ := core.AsInteger(st[0]); n != 6 {
	...
	st, fired, err = vc.deoptIfFn(r, fn.Body, false, &spec, 1, []core.Value{core.NewInteger(0), five, inc}, nil, seam7Dbg, 0)
	if err != nil || !fired || len(st) != 2 {
	...
	st, fired, err = vc.deoptIfFn(r, fn.Body, false, &pre, 0, []core.Value{inc}, []core.Value{five}, seam7Dbg, 0)
	if err != nil || !fired || len(st) != 1 {
	...
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &tail, 0, []core.Value{inc}, nil, seam7Dbg, 0); err == nil || !strings.Contains(err.Error(), "uncalled_function") {
	...
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: -1, Depth: -1, Results: 2, Token: 2, RetPC: 9}, 1, []core.Value{inc}, nil, seam7Dbg, 0); err == nil {
		t.Error("results below the frame base raise")
	}
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: -1, Depth: -1, Results: 1, Token: 3, RetPC: 9}, 0, []core.Value{inc}, nil, seam7Dbg, 0); err == nil {
		t.Error("a token beyond the body raises")
	}
	if _, _, err := vc.deoptIfFn(r, fn.Body, false, &compiler.DeoptSpec{Slot: -1, Depth: -1, Prefix: []int{3}, Results: 1, Token: 2, RetPC: 9}, 0, []core.Value{inc}, nil, seam7Dbg, 0); err == nil {
