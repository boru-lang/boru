...
// into the unit call's capture compile failure ("capture f of  unreachable at a
...
func TestTailApplyCollapseParity(t *testing.T) {
	rows := []struct{ src, note string }{
		{`def bb f:Function => [g:Function => [x:Any => [(x g/v apply) f/v apply]]] end 4 ((n:Integer => [add n 3]) (bb (n:Integer => [mul n 2])) apply) apply`, "14 — B with inline-lambda args (ledger row, was declined)"},
...
// TestTailApplyCollapseSoundCompileFailures pins the neighbours that still DECLINE.
func TestTailApplyCollapseSoundCompileFailures(t *testing.T) {
...
// before the collapse landed: every such call site declined). The unbound
