...
// dropped_apply_guard_test.go pins the def site's DROPPED-APPLY compile failure
...
// Decline. The program is then silently re-run on the interpreter —
...
// arm — no compile failure of its own), and the paren places the callee unchanged.
...
// FIRST compile failure on the pass — no earlier model has anything to say here.
...
// TestDroppedApplyDefCompileFailure pins the compile failure itself: the second def binds a
...
// really produced.
func TestDroppedApplyDefCompileFailure(t *testing.T) {
	...
	for _, c := range rows {
		...
		if prog != nil {
			t.Errorf("%q: compiled — expected the dropped-apply compile failure (%s)", c.src, c.note)
			continue
		}
		if !strings.Contains(reason,
			"def of a computed fn whose apply the analysis dropped") {
			t.Errorf("%q: declined %q, want the dropped-apply reason (%s)", c.src, reason, c.note)
		}
...
// TestDroppedApplyDefCompileFailureNeighbours pins the two negatives: the SAME
// unclaimed wrapper applied without a second def declines somewhere else
...
func TestDroppedApplyDefCompileFailureNeighbours(t *testing.T) {
	// No second def: nothing rebinds fs's carrier, so the dropped-apply
	// guard stays silent and the flattened residual owns the compile failure.
	a, err := New()
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(reason, "closure shape unknown") {
		t.Errorf("%q: declined %q, want the residual classifier's reason", bare, reason)
	}
	if strings.Contains(reason, "the analysis dropped") {
		t.Errorf("%q: declined with the dropped-apply reason, but no def rebinds the carrier", bare)
	}
