...
func init() {
	registerDocs("boru:io", map[string]string{
		...
		"move":      "Rename/move a path, returning the destination; {overwrite:false} declines an existing one.",
		"copy":      "Copy a path to a destination; {recursive} copies a tree, {overwrite:false} declines an existing one.",
