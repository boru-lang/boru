...
// declined for a concrete argument inside a predicate body precisely
// because it is declined at top level, and the outer `is` / `unify`
...
// declined. This pins the consistent rule on both engines.
func TestPredicateBodyDispatchIsUnarmedLikeTopLevel(t *testing.T) {
	...
	// The top-level call the body makes is declined the same way — that
	...
	for _, err := range []error{errC, errI} {
		if err == nil || !strings.Contains(err.Error(), "no signature matches the arguments") {
			t.Fatalf("%q: top-level dispatch must be declined, got %v", top, err)
		}
	}
}
