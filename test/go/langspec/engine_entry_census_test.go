...
const deferCeiling = 0 // the REGRESSION ceiling (lanes_test.go; end state 0): 3 -> 0 on 2026-09-26 (the merge of main's #511 with the reverse-order NUR run, measured on the merged tree): main's flex write shapes (5 -> 2) close the run's three vm:poly-nout-drift (flex.tsv L228/L230/L236), and main's two left, vm:landing-quote-claim×2 (fn-value.tsv L317/L318), the run had closed with NUR190; left: none. Main's #511: 5 -> 2 on 2026-09-26 (the flex write shapes): flex.tsv L228/L230/L236, `set` on a flex member, commit the FlexMap overload and its one result through the member read's recorded write shape where the dynamic(Any) member committed the Class overload's none and bailed at vm:poly-nout-drift×3. Before: 5 -> 3 on 2026-09-26 (the merge of main's #510 with the reverse-order NUR run): the two sides' 7 -> 5 moved different rows, so they compose — main's convert-ideal.tsv L33 and edge-scalars-3.tsv L218 (vm:poly-no-match×2) and the NUR run's fn-value.tsv L317/L318 (vm:landing-quote-claim×2, NUR190 closed); left: vm:poly-nout-drift×3 (flex.tsv L228/L230/L236). Main's history: 7 -> 5 on 2026-09-26 (the type-name reach): convert-ideal.tsv L33 and edge-scalars-3.tsv L218, `convert Integer <x>` with no overload for the operand, raise the interpreter's signature_error through the poly no-match plan (the reach bound counts a builtin type-name word as the one value it steps to) where they bailed at vm:poly-no-match×2; left: vm:poly-nout-drift×3 (flex.tsv L228/L230/L236), vm:landing-quote-claim×2 (NUR190, by choice). The NUR run's history: 7 -> 5 on 2026-09-26 (NUR190 closed): fn-value.tsv L317/L318 — `m.f z`, `m get 'f' z` — no longer bail at the landing (vm:landing-quote-claim): the `/q` capture takes the landing's island. Before: 10 -> 7 on 2026-09-25 (the strict-Any dyn-body recovery): fold-map-filter.tsv L246–L248, fold/each/scan over a class member's Any-typed list, run natively through the dyn-body poly re-match the recovery records instead of bailing at the rematch trap (vm:rematch-matched×3 gone); before: 8 -> 10 on 2026-09-24 (NUR190's open halves deferred, the maintainer's call): fn-value.tsv:L317/L318 (`m.f z`, `m get 'f' z` — a `/q` slot CAPTURES the following word) BAIL at the landing's walk (vm:landing-quote-claim×2) where they passed by coincidence (z's result is its own atom) and the same shape answered wrong off-corpus (`m.f y` was `[42 42]` for `[y]`); a compile-time decline would have been over-wide (no static model tells a `/q` slot from a typed slot's barrier), and the deferral is kept on the per-file runtime-defers ledger (runtime_defers.tsv, runtime_defer_ledger_test.go) as the maintainer asked. Before: 8 on 2026-09-17 — vm:poly-nout-drift×3, vm:rematch-matched×3 (the corpus expansion), vm:poly-no-match×2. History: 5 // 5 (2026-08-25, Stage-1 baseline) -> 0 (Stage 9)
