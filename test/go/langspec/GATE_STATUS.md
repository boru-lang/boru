...
| compile failures | 16 | 0 | 16 | open | corpus rows that FAIL to compile — every one a BUG, not a policy (design/COMPILABLE-SUBSET.md §5); the sum of compile_failures.tsv |
| compute gaps | 11 | 0 | 11 | open | real-compute rows that fail to compile |
...
| engine entries | 166 | 0 | 166 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×166, CallBoru×132, RunResolved×16, vm:island-resolved×7, InvokeCallback:callboru×5, runPooledSub×5, vm:island×4 |
| interp-entry census rows | 24 | 0 | 24 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
