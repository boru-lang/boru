...
| engine entries | 392 | 0 | 392 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×392, CallBoru×239, RunResolved×90, runPooledSub×26, vm:island-resolved×10, vm:island×6, InvokeCallback:callboru×5 |
| interp-entry census rows | 68 | 0 | 68 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
