...
| engine entries | 172 | 0 | 172 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×172, CallBoru×132, RunResolved×16, runPooledSub×9, vm:island-resolved×7, InvokeCallback:callboru×5, vm:island×4 |
| interp-entry census rows | 26 | 0 | 26 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
