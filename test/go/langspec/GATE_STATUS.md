...
| engine entries | 344 | 0 | 344 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×344, CallBoru×239, RunResolved×67, runPooledSub×10, vm:island-resolved×10, vm:island×6, InvokeCallback:callboru×5 |
| interp-entry census rows | 57 | 0 | 57 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
