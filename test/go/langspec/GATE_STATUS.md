...
| engine entries | 335 | 0 | 335 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×335, CallBoru×235, RunResolved×67, runPooledSub×10, vm:island-resolved×7, InvokeCallback:callboru×5, vm:island×4 |
| interp-entry census rows | 53 | 0 | 53 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
