...
| compile failures | 32 | 0 | 32 | open | corpus rows that FAIL to compile — every one a BUG, not a policy (design/COMPILABLE-SUBSET.md §5); the sum of compile_failures.tsv |
| compute gaps | 27 | 0 | 27 | open | real-compute rows that fail to compile |
...
| engine entries | 416 | 0 | 416 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×416, CallBoru×241, RunResolved×106, runPooledSub×26, vm:island×13, vm:island-resolved×9, InvokeCallback:callboru×7 |
