...
| compile failures | 23 | 0 | 23 | open | corpus rows that FAIL to compile — every one a BUG, not a policy (design/COMPILABLE-SUBSET.md §5); the sum of compile_failures.tsv |
| compute gaps | 18 | 0 | 18 | open | real-compute rows that fail to compile |
...
| engine entries | 415 | 0 | 415 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×415, CallBoru×242, RunResolved×103, runPooledSub×26, vm:island×13, vm:island-resolved×10, InvokeCallback:callboru×7 |
| interp-entry census rows | 77 | 0 | 77 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
