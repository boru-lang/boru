...
| engine entries | 375 | 0 | 375 | open | unattributed interpreter runs on the compiled path, by seam: Engine.Run×375, CallBoru×239, RunResolved×73, runPooledSub×26, vm:island-resolved×10, vm:island×6, InvokeCallback:callboru×5 |
| interp-entry census rows | 62 | 0 | 62 | open | corpus rows that run compiled and still enter the interpreter through an unattributed seam — the OpFallback island ceiling cannot see this (it counts disassembly spans, not a CallBoru inside a handler) |
