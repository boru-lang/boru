...
// count is pinned with a floor so a regression that silently declines
...
// is guarding — the rule each emitter compile failure is defending.
package langspec
...
func TestSpecCompiledDifferential(t *testing.T) {
	...
	if !filteredCorpus() && compiled < minCompiledRows {
		t.Errorf("only %d rows took the compiled path (floor %d) — the emitter regressed to declining the corpus",
