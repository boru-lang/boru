...
Corpus: **8552** spec value rows (**8213** compilable, 339 statically invalid in both engines).

## Disposition

| outcome | rows |
| --- | ---: |
| compiled natively (fallback-free) | 8151 |
| compiled with an interpreter island | 0 |
| does not compile | 62 |
| static check-error (invalid in both engines) | 339 |

**8151 / 8213** compilable rows produce a Program (99% — 8151 of those fully native).
...
Over the 62 not-fully-native rows (declined or islanded): **0** interpreter-only (tier 1, permanent), **3** reducible (tier 2, TODO), **1** allowlisted error rows, **58** compute-frontier gaps.
