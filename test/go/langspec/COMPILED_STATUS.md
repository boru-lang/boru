...
Corpus: **8513** spec value rows (**8174** compilable, 339 statically invalid in both engines).

## Disposition

| outcome | rows |
| --- | ---: |
| compiled natively (fallback-free) | 8121 |
| compiled with an interpreter island | 0 |
| does not compile | 53 |
| static check-error (invalid in both engines) | 339 |

**8121 / 8174** compilable rows produce a Program (99% — 8121 of those fully native).
...
Over the 53 not-fully-native rows (declined or islanded): **0** interpreter-only (tier 1, permanent), **3** reducible (tier 2, TODO), **1** allowlisted error rows, **49** compute-frontier gaps.
