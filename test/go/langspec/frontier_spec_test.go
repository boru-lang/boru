...
var frontierCompileLedger = map[string]frontierEntryLS{
	...
	`def bb f:Function => [g:Function => [x:Any => [(x g/v apply) f/v apply]]] end 4 ((n:Integer => [add n 3]) (bb (n:Integer => [mul n 2])) apply) apply`:                                                                                                                                                                                                                                                                                                 {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's `apply` over the produced closure records now (a pending application the re-step's dispatch takes through RecordDynApply), so the refusal MOVED to the x-level closure's dispatch — its call-site residual is TWO values, the gradual `(x g/v apply)` result and the fn-typed capture `f` its tail `apply` consumes at the unit's finish, so the single-out record site declines and the unit call refuses the construction-scope capture; graduation = collapsing a unit's pending tail apply into its call-site residual. Earlier diagnosis: audit §5.8 over B; the staged apply leaves a computed closure at the word's argument slot", failsWith: "unreachable at a call site"},
	`def bb f:Function => [g:Function => [x:Any => [(x g/v apply) f/v apply]]] end def d fn n:Integer Integer [mul n 2] end def e fn n:Integer Integer [add n 3] end 4 (e/v (bb d/v) apply) apply`:                                                                                                                                                                                                                                                         {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's `apply` over the produced closure records now (a pending application the re-step's dispatch takes through RecordDynApply), so the refusal MOVED to the x-level closure's dispatch — its call-site residual is TWO values, the gradual `(x g/v apply)` result and the fn-typed capture `f` its tail `apply` consumes at the unit's finish, so the single-out record site declines and the unit call refuses the construction-scope capture; graduation = collapsing a unit's pending tail apply into its call-site residual. Earlier diagnosis: audit §5.8 over B; the named-/v call-site twin (checks clean at the CLI — the NUR089 pair)", failsWith: "unreachable at a call site"},
	...
	`def cpair a:Any => [b:Any => [s:Function => [b/v (a/v s/v apply) apply]]] end def cfst p:Function => [(a:Any => [b:Any => [a/v]]) p/v apply] end def p (2 (cpair 1) apply) end (cfst p/v)`:                                                                                                                                                                                                                                                            {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's apply over a produced closure records now, so the refusal MOVED into the projection's body: `(a:Any => [b:Any => [a/v]]) p/v apply` puts a lambda VALUE beneath the fn-typed param's pending apply, and the unit finish's whole-residual lowering declines a fn-valued window entry (argsOK); behind it `def p (2 (cpair 1) apply)` binds a computed fn and `(cfst p/v)` hands a produced closure to a user fn's Function slot (the argument-slot family); graduation = admitting a fn value as DATA beneath a pending tail apply (the re-step already matched it), then the def-bound computed fn. Earlier diagnosis: RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church pair, first projection", failsWith: "apply of a dynamic fn value not at the body tail"},
	`def cpair a:Any => [b:Any => [s:Function => [b/v (a/v s/v apply) apply]]] end def csnd p:Function => [(a:Any => [b:Any => [b/v]]) p/v apply] end def p (2 (cpair 1) apply) end (csnd p/v)`:                                                                                                                                                                                                                                                            {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's apply over a produced closure records now, so the refusal MOVED into the projection's body: `(a:Any => [b:Any => [a/v]]) p/v apply` puts a lambda VALUE beneath the fn-typed param's pending apply, and the unit finish's whole-residual lowering declines a fn-valued window entry (argsOK); behind it `def p (2 (cpair 1) apply)` binds a computed fn and `(cfst p/v)` hands a produced closure to a user fn's Function slot (the argument-slot family); graduation = admitting a fn value as DATA beneath a pending tail apply (the re-step already matched it), then the def-bound computed fn. Earlier diagnosis: RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church pair, second projection", failsWith: "apply of a dynamic fn value not at the body tail"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cnot p:Function => [t:Any => [f:Any => [t/v (f/v p/v apply) apply]]] end 'F' ('T' (cif (cnot ctrue/v)) apply) apply`:                                                                                                                                                                 {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's apply over a produced closure records now; this row's refusal is `(cnot ctrue/v)` at cif's `p:Function` ARGUMENT slot — argIsProducedClosure's word-agnostic hold, which reads every produced closure at a slot as an uncollapsed paren, where a user fn's declared Function slot binds it as data; graduation = admitting a produced closure at a user fn's Function slot. Earlier diagnosis: RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church not true", failsWith: "computed closure at a word's argument slot"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cnot p:Function => [t:Any => [f:Any => [t/v (f/v p/v apply) apply]]] end 'F' ('T' (cif (cnot cfalse/v)) apply) apply`:                                                                                                                                                                {why: "RE-DIAGNOSED 2026-09-08 (the twenty-eighth increment): the main program's apply over a produced closure records now; this row's refusal is `(cnot cfalse/v)` at cif's `p:Function` ARGUMENT slot — argIsProducedClosure's word-agnostic hold, which reads every produced closure at a slot as an uncollapsed paren, where a user fn's declared Function slot binds it as data; graduation = admitting a produced closure at a user fn's Function slot. Earlier diagnosis: RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church not false", failsWith: "computed closure at a word's argument slot"},
