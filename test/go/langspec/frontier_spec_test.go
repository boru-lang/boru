...
var frontierCompileLedger = map[string]frontierEntryLS{
	...
	`def factk fn [[n:Integer k:Function][Any][ if (lte 1 n) [ 1 k/v apply ] [ def kk ( fn r:Integer Any [ def m (mul n r) m k/v apply ] ) (factk (sub 1 n) kk/v) ] ]] end (factk 5 (v:Integer => [v]))`:                                                                                                                                                                                                                                                   {why: "RE-DIAGNOSED 2026-09-08 (the thirty-third increment): the `/v` read of the fn-local capturing literal `kk` handed to factk resolves now (its closure operand), so the refusal MOVED to the then-arm `[ 1 k/v apply ]`: a pending apply over the k:Function param inside a branch ARM is not the unit's body tail. Graduation = an arm-tail apply window (and its gradual result at the arm's lead, which residualLeadReStepped holds)", failsWith: "apply of a dynamic fn value not at the body tail"},
	`def factk fn [[n:Integer k:Function][Any][ if (lte 1 n) [ 1 k/v apply ] [ def kk ( fn r:Integer Any [ def m (mul n r) m k/v apply ] ) (factk (sub 1 n) kk/v) ] ]] end (factk 10 (v:Integer => [add 0 v]))`:                                                                                                                                                                                                                                            {why: "audit §1.3/§5.8: same CPS shape, deeper recursion — the then-arm apply (see the first CPS row, the thirty-third increment)", failsWith: "apply of a dynamic fn value not at the body tail"},
