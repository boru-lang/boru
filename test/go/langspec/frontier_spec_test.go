...
var frontierCompileLedger = map[string]frontierEntryLS{
	...
	`def app fn [[nd:Any m:Map] [Any] [nd (m get "inc") apply]] def rules {inc: ([x:Integer] => [x add 1])} app 5 rules`:                                                                                     {why: "the fetched-fn apply COMPILES since 2026-09-07 (the twenty-seventh increment: apply over a gradual lead lowers to OpCallDynApplyOne, and its negative twin graduated), but the map-member lambda `rules.inc` is never STAMPED — a lambda inside a map literal is baked as a const with no compiled unit — so the apply word's re-step runs its body on the interpreter (vm:island-resolved), the interp-entry census's debt; graduation = stamping a fn VALUE on its first apply (StampDetachedFn from the op), which enters the unit VM-native", failsWith: "islanded"},
	...
	`def kk x:Any => [y:Any => [x/v]] end def ss f:Function => [g:Function => [x:Any => [(x g/v apply) (x f/v apply) apply]]] end def ii (kk/v (ss kk/v) apply) end 42 ii/v apply`:                 {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: curried combinator body result is an inner fn literal — unknown provenance (respelled for BROAD, NUR073)", failsWith: "computed closure at a word's argument slot"},
	`def kk x:Any => [y:Any => [x/v]] end def ss f:Function => [g:Function => [x:Any => [(x g/v apply) (x f/v apply) apply]]] end def ii (kk/v (ss kk/v) apply) end 'hello' ii/v apply`:            {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: same program over a String operand", failsWith: "computed closure at a word's argument slot"},
	...
	`def cc f:Function => [x:Any => [y:Any => [x/v (y f/v apply) apply]]] end def add2 a:Integer => [b:Integer => [add a b]] end 10 (1 (cc add2/v) apply) apply`:                                   {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8 over C (argument flip)", failsWith: "computed closure at a word's argument slot"},
	...
	`def ww f:Function => [x:Any => [x/v (x f/v apply) apply]] end def add2 a:Integer => [b:Integer => [add a b]] end 4 (ww add2/v) apply`: {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8 over W (argument duplication)", failsWith: "computed closure at a word's argument slot"},
	...
	`def cpair a:Any => [b:Any => [s:Function => [b/v (a/v s/v apply) apply]]] end def cfst p:Function => [(a:Any => [b:Any => [a/v]]) p/v apply] end def p (2 (cpair 1) apply) end (cfst p/v)`:                                                                                                                                                                                                                                                            {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church pair, first projection", failsWith: "computed closure at a word's argument slot"},
	`def cpair a:Any => [b:Any => [s:Function => [b/v (a/v s/v apply) apply]]] end def csnd p:Function => [(a:Any => [b:Any => [b/v]]) p/v apply] end def p (2 (cpair 1) apply) end (csnd p/v)`:                                                                                                                                                                                                                                                            {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church pair, second projection", failsWith: "computed closure at a word's argument slot"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end 'F' ('T' (cif ctrue/v) apply) apply`:                                                                                                                                                                                                                                                                                              {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church true", failsWith: "computed closure at a word's argument slot"},
	`def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end 'F' ('T' (cif cfalse/v) apply) apply`:                                                                                                                                                                                                                                                                                            {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church false", failsWith: "computed closure at a word's argument slot"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cnot p:Function => [t:Any => [f:Any => [t/v (f/v p/v apply) apply]]] end 'F' ('T' (cif (cnot ctrue/v)) apply) apply`:                                                                                                                                                                 {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church not true", failsWith: "computed closure at a word's argument slot"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cnot p:Function => [t:Any => [f:Any => [t/v (f/v p/v apply) apply]]] end 'F' ('T' (cif (cnot cfalse/v)) apply) apply`:                                                                                                                                                                {why: "RE-DIAGNOSED 2026-09-07 (the twenty-seventh increment): the combinator's returned lambda — an `apply`-word chain over gradual leads — now COMPILES (apply over a gradual lead lowers to OpCallDynApplyOne), so the refusal MOVED to the MAIN program's `apply` over the produced closure, the same Stage-2 argument-slot refusal the staged B/K rows hold; graduation = the apply word over a produced closure at the main program. Earlier diagnosis: audit §5.8: Church not false", failsWith: "computed closure at a word's argument slot"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cand p:Function => [q:Function => [cfalse/v (q/v p/v apply) apply]] end 'F' ('T' (cif (cfalse/v (cand ctrue/v) apply)) apply) apply`:                                                                                                                                                 {why: "audit §5.8: Church and — NOT moved by the twenty-seventh increment: cand's inner lambda applies a def-bound lambda read by /v (`cfalse/v (q/v p/v apply) apply`), the NUR101 def-read hold, so its own unit still refuses; T and F", failsWith: "body result of unknown provenance"},
	`def ctrue t:Any => [f:Any => [t/v]] end def cfalse t:Any => [f:Any => [f/v]] end def cif p:Function => [t:Any => [e:Any => [e/v (t/v p/v apply) apply]]] end def cand p:Function => [q:Function => [cfalse/v (q/v p/v apply) apply]] end 'F' ('T' (cif (ctrue/v (cand ctrue/v) apply)) apply) apply`:                                                                                                                                                  {why: "audit §5.8: Church and — NOT moved by the twenty-seventh increment: cand's inner lambda applies a def-bound lambda read by /v (`cfalse/v (q/v p/v apply) apply`), the NUR101 def-read hold, so its own unit still refuses; T and T", failsWith: "body result of unknown provenance"},
