...
var refusalDispositions = map[string]refusalDisposition{
	...
	"compiler/go/emit.go:refuseUndef#1":                               {dispGeneric, 4, "the undef hooks' one site (RefuseCarriedUndef, RefuseSpeculativeUndef, RecordSpeculativeUndef's declines, the def-after-undef, forward-slot and unseated-read guards): a speculative undef of a module-scope value binding is PLACED since the sixty-eighth increment (OpUndefDynScope, live reads seated at their tokens), and a forward word slot of the name ROUTES since the sixty-ninth; what still refuses — an undef of a carried name, of a type, fn-family or frame binding, one the recorder cannot seat (a suspended recording, an arm-resident bracket), a def of the name inside its region, a forward slot the op cannot drive, a `/v` read the hook does not seat — is the resident bridge's undef pairing and the region host's evaluations"},
