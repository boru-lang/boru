...
var refusalDispositions = map[string]refusalDisposition{
	...
	"compiler/go/emit.go:refuseUndef#1":                               {dispGeneric, 4, "the undef hooks' one site (RefuseCarriedUndef, RefuseSpeculativeUndef, RecordSpeculativeUndef's declines, the def-after-undef and forward-slot guards): a speculative undef of a module-scope value binding is PLACED since the sixty-eighth increment (OpUndefDynScope, live reads); what still refuses — an undef of a carried name, of a type, fn-family or frame binding, one the recorder cannot seat (a suspended recording, an arm-resident bracket), a def of the name inside its region, a forward-slot read of it — is the routed dispatch's unbound-slot arm and the resident bridge's undef pairing"},
