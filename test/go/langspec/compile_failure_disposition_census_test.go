// The compile failure-disposition census — every compile failure site has a plan.
//
...
//     mechanism itself, and retires when compile failure does (section 6.10).
//
...
// so, which is the intended cost: every change to the compile failure surface
// re-states its plan.
//
...
var compileFailureDispositions = map[string]compileFailureDisposition{
	...
	"compiler/go/emit.go:recordLoopEvent#3":                           {dispDelete, 9, "an internal invariant (the loop setup registers the slot): a structured internal_error, never a compile failure"},
	...
	"compiler/go/emit.go:recordCallCompileFailure#1":                  {dispGeneric, 4, "dispatch without a signature: the generic lane's lookup"},
	"compiler/go/emit.go:recordCallCompileFailure#2":                  {dispGeneric, 3, "anonymous function dispatch: the Apply kernel"},
	"compiler/go/emit.go:recordCallCompileFailure#3":                  {dispGeneric, 7, "a check-mode word's runtime effect: compile the expansion when it is produced"},
	"compiler/go/emit.go:recordCallCompileFailure#4":                  {dispGeneric, 3, "a user fn call without a unit: universal fn values"},
	"compiler/go/emit.go:recordCallCompileFailure#5":                  {dispGeneric, 5, "a full-stack word beyond FoldFullStack's exactness: a region"},
	"compiler/go/emit.go:recordCallCompileFailure#6":                  {dispGeneric, 3, "a fn value read from a container auto-dispatching: the arrival model"},
	"compiler/go/emit.go:recordCallCompileFailure#7":                  {dispGeneric, 6, "a context-dependent word: a per-region context frame"},
	"compiler/go/emit.go:recordCallCompileFailure#8":                  {dispGeneric, 6, "a code-body word: units, not tokens (the handler migration)"},
	"compiler/go/emit.go:recordCallCompileFailure#9":                  {dispGeneric, 6, "a quoted-operand word: the modifier words lower to a dispatch"},
	"compiler/go/emit.go:recordCallCompileFailure#10":                 {dispGeneric, 4, "core-default dispatch over a carrier: OpDispatchGeneric"},
	"compiler/go/emit.go:recordCallCompileFailure#11":                 {dispGeneric, 4, "a dynamic input: OpDispatchGeneric"},
	"compiler/go/emit.go:recordCallCompileFailure#12":                 {dispGeneric, 6, "an unannotated or opaque word: the declaration triple, every signature declares"},
	"compiler/go/emit.go:recordCallCompileFailure#13":                 {dispGeneric, 5, "a computed range list: FOR_SETUP over a live list"},
	...
	"compiler/go/emit.go:RecordPolyCall#1":                            {dispGeneric, 3, "as recordCallCompileFailure#6, for the poly call"},
	...
	// test/specfix — fixtures of the compile failure path, counted by the site census.
	"test/specfix/control.go:fixIf3ReturnsFn#1": {dispDelete, 9, "a fixture of the compile failure path; retires with it"},
	"test/specfix/control.go:fixIf3ReturnsFn#2": {dispDelete, 9, "a fixture of the compile failure path; retires with it"},
}
...
const compileFailureDispositionCeiling = 91 // 90 -> 91 on 2026-09-26 (the merge of main's #512): main's basic/go/native_control.go:ifTakenArmReturns#1 — a decided arm that leaves a fn value declines, closing main's live miscompile (`10 if [true] [g/v] [1]` applied g to the 10 beneath); on main the site moved out of if3ReturnsFn#1, which this run had retired for its 0-netting half (NUR243). Before: 91 -> 90 on 2026-09-26 (NUR243): basic/go/native_control.go:if3ReturnsFn#1 retired, the disposition its row prescribed — a constant branch's taken arm that leaves no value is recorded as a 0-value statement (a phantom None result, the event zeroOut), as the both-arms-zero branch is. Before: 92 -> 91 on 2026-09-26 (NUR231's type half): core/go/depscalar.go:DeclineUnknownRefinement#1 retired, the disposition its row prescribed — a named type over a computed bound installs at run time (OpBindTypeRun, its node forwarded to the run's) and a typed def records the run's membership check; a signature the run builds (an inline type over such a bound) declines through the existing compile-time-word site. Before: 91 -> 92 the same day (NUR231): the site landed, a type over a refinement whose bound only the run knows having compiled silently wrong. Before: 92 -> 91 on 2026-09-24 (the selective freshen): compiler/go/emit.go:resolveOperand#1 retired — a body literal embedding an enclosing binding's container no longer declines; the fresh push clones the literal's spine and keeps the embedded member (Program.ConstKeep), the disposition the row prescribed. Before: 92 (2026-09-14, the census's first cut) -> 0 (Stage 9)
...
func dispositionFindings(keys []string, table map[string]compileFailureDisposition, ceiling int) (findings []string, byKind map[string]int, byStage map[int]int) {
	...
	for _, k := range keys {
		...
		d, ok := table[k]
		if !ok {
			findings = append(findings, "compile failure site "+k+" has no disposition — under the definition of done every site needs one of generic, trap or delete")
			continue
		}
		switch d.kind {
		case dispGeneric, dispTrap, dispDelete:
		default:
			findings = append(findings, "compile failure site "+k+": disposition "+d.kind+" is not one of the three the ruling permits")
		}
		if d.stage < 3 || d.stage > 9 {
			findings = append(findings, "compile failure site "+k+": stage "+itoa(d.stage)+" is not a FULL-COMPILATION.0.md section 10 stage")
		}
		if strings.TrimSpace(d.note) == "" || strings.Contains(d.note, "\n") {
			findings = append(findings, "compile failure site "+k+": the note must be one non-empty line naming the mechanism that retires the site")
		}
		byKind[d.kind]++
		byStage[d.stage]++
	}
	for k := range table {
		if !found[k] {
			findings = append(findings, "disposition table entry "+k+" names no compile failure site — the site retired, so drop the row (or the key moved with a reordering: re-key it)")
		}
	}
	switch {
	case len(keys) > ceiling:
		findings = append(findings, "compile failure-disposition census "+itoa(len(keys))+" exceeds ceiling "+itoa(ceiling)+" — a new compile failure site was added with its row; the count only falls, so the site needs a generic lowering, a trap or a deletion instead")
	case len(keys) < ceiling:
		findings = append(findings, "compile failure-disposition census "+itoa(len(keys))+" is below ceiling "+itoa(ceiling)+" — a site retired: lower the ceiling and record which, so the history stays exact")
	}
...
func TestCompileFailureDispositionCensus(t *testing.T) {
	...
	t.Logf("compile failure-disposition census: %d sites — generic %d, trap %d, delete %d; by retiring stage: %s",
...
func TestCompileFailureSiteKeysAreSyntactic(t *testing.T) {
	t.Parallel()
