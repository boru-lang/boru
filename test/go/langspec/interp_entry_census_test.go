...
// therefore not debt. The stack-form recorder's run is the same kind
// ("stackform-record", lang/go/stackform Compile): the PBT shrinker records
// the ENGINE's pushes and dispatches to build a form it then rewrites, an
// observation the VM cannot supply, and the run's own answer is discarded. It is NOT an escape hatch — the test is whether COMPILING
