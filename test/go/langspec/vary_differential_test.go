...
var varyRefusalLedger = map[string]string{
	"islanded": "a full-stack word (pick/depth/roll) over EVENT-PRODUCED entries inside a wrapped code body compiles WITH an OpFallback island — sound (interpreter re-entry, parity held). NARROWED 2026-09-11 (the fifty-second increment): the fold IS per-unit now, and the const/local occurrence folds in the body's own unit (graduated to corpus-core.tsv). What keeps this half islanded is not exactness but the absence of a residual REBUILD in a body unit: the top unit can take a permuted residual (seatResidualRebuild), a body unit's seating refuses a result above a literal, and folding a permutation there turns this island into a REFUSAL — which this very test measured on the increment's first cut. Graduation = the residual rebuild inside a body unit. Representative row: frontier-full-stack.tsv",
