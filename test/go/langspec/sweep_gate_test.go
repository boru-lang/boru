...
const (
	...
	sweepFailureCeiling        = 0   // valid seeds that FAIL to compile or hard-error in CompileCheck — every one a BUG. 1 -> 0 on 2026-09-26 (`if` × container, NUR207): `def m {f: ([] => [1])} end if true m.f [2]` compiles with parity — a get / dot read of a PARKING member (an anonymous, capture-free, unapplied, only-0-arg fn: every value landing parks it) over a concrete container and key folds to the member value itself (compiler tryFoldParkedMemberFn, the map twin of tryFoldStaticIndex's list fold), so the read is the lambda literal on both passes and a later NAME read, `apply` or param read dispatches it as a literal's does (`def j (m get "f")  j` is 42); the dynamic carrier the 0-arg landing left, which hid the fn from those readers, is gone. The family standing aside exposed was measured and narrowed first: a def-bound read of a branch result an arm of which leaves a fn declines (armLeavesFn), an `Any`-returning factory's result and a pinpointed arg-taking member claim their shape at the def (the read model dispatches the name), `apply` over a gradual lead with a structured producer is the pending apply, and a gradual carrier proven to hold a fn declines at a user fn parameter not typed Function; what is left of NUR207's family is recorded (NUR216–NUR218). Before: 3 -> 1 on 2026-09-26 (behave × container and fnsig × module-export): `behave` × container compiles with parity — behave declares CompileDynBody and CompileFnHandlerStrict, and the dyn-body backstop records the gradual `m.c` operand as a poly re-match (recordStoredFnDyn) once the member is PROVEN to arrive as an interpreter fn value (a read over a const container whose member is a concrete fn — strictFnOperandProven; a closure-valued, flex or param member keeps "dynamic input at behave", since behave cannot install a compiled closure's body), with DynEnv armed when the stored body names something; and `fnsig` × module-export (the check-reject) compiles — `def T fnsig M.sg` mints NOTHING on the check engine (binding the carrier is what miscompiled `is T`): the def form binds nothing there and its dispatch is emitted as the plain CALL_NATIVE it is (NoteRuntimeDefDispatch → RecordRuntimeBindDispatch, unpack's run-time-bind seam), so the run constructs and installs T exactly as the interpreter does; a later static read of T is the pass's undefined-word finding and declines, and the form is fenced to the root stream and to a name part no later type of the pass registers. Left: `if` × container (the anonymous 0-arg member's landing — NUR207). Before: 13 -> 3 on 2026-09-26 (the sweep's last cells): ten failing seeds compile with parity — `force-arity` / `forward-args` / `usurp` / `stack-args` × factory (the modifier natives wrap a compiled closure over its bridged shape and the VM applies the closure the wrapper stores, so the typed-carrier poly record is admitted — NUR158 FIXED), `if` × lambda (a branch of render-known fn values is placed data at the residual), `emit` / `mini` × factory, `mini` / `parse` × container (both check-rejects — a gradual leading operand no longer raises "the kind must be a literal name"; the compile pass records the module's runtime fn-dispatch native, which takes the interpreter's route for the value it finds) and `receive` × module-export (a computed clause list takes the dyn-body backstop; receive declares CompileDynBody). Left, each a named cause: `behave` × container (dynamic input at behave — a gradual member fn a closure could replace at run time, which behave cannot install), `if` × container (the anonymous 0-arg member's landing — standing it aside was measured to miscompile a later name read, NUR207) and `fnsig` × module-export (a type minted from a runtime list; binding the carrier miscompiles `is T`). The five DIVERGED seeds that also left — `codequote` × literal / lambda / factory and `word` × container (bails) and `emit` × container (NUR170, FIXED) — were never in this count. Before: 17 -> 13 on 2026-09-25 (behave's quoted behaviour name declared CompileQuoteInert): `behave` × literal, named-fn, factory and module-export compile with parity — the atom is a table key the handler reads verbatim and the fn operand already baked as an inert stored fn, so the dispatch is a plain CALL_NATIVE that installs the behaviour on the run-time registry's type; × container (a fn read from a map member) still declines. Before: 25 -> 17 on 2026-09-25 (the fn-value callback cells): `for-each` × factory / container / module-export and `walk` × factory / container / module-export compile with parity — both words declare CompileDynBody now, and tryRecordDynBody admits for-each's 0-result dispatch (BodyOut 0 is the word's own count), so a callback the closure path cannot compile (a factory's result, a container member, a module export) rides as a runtime value into a CALL_NATIVE under DynEnv exactly as each's twins have since S1a; and `codequote` × container / module-export (`typeof (codequote m.f)`, `typeof (codequote M.inc)`) compile — a QUOTED receiver reach whose tokens are inert members bakes as an inert const (isInertReach), since neither engine expands a quoted reach. Before: 29 -> 25 on 2026-09-24 (NUR200 closed, the multi-run keep-defs body): the `for-each` and `walk` seeds, literal and computed (`def acc (flex [])  for-each [acc swap push] [1 2 3]  size acc` and their quoted-body twins), compile — a flex a multi-run body MUTATES is read back after it as a live lookup of the registry's cell (the pass holds a re-modelled carrier with no compiled home; NoteLiveRead's mutable-ref arm). Before: 30 -> 29 on 2026-09-23 (NUR181, the def-bound closure park): the `word` × factory seed (`def dbl word (mk) end 5 dbl`) compiles with parity — the program residual's ordering treats a parked result as data, and a splice's payload is marked re-stepped so the trailing arm applies it. Before: 31 -> 30 on 2026-09-22 (the dynamic-lead group): the `apply` × container seed (`def m {f: ([n:Integer] => [n add 1])} end 5 m.f/v apply`) compiles — the gradual apply EVENT records at the program level too — and all fourteen of its call-form variants pass with it. Before: 44 -> 36 on 2026-09-19 (S1a): each/fold/scan/filter × factory and × container poly re-match. 36 -> 31 on 2026-09-19 (the fallback removal): five seeds that used to be classified as failures now compile and RUN — the classifier read the try-mode fallback's error as a compile failure, and with one outcome it reads the real one
...
var sweepKnownMiscompiles = map[string]sweepPin{
	// Surfaced 2026-09-25 by the fn-value callback cells graduating (the
	// for-each and walk module-export seeds carry module STATE, `acc`):
	// the mechanism is older than either — an inline `import module […]`
	// inside a loop body runs once compiled, and `for 2 [import module
	// [def acc (flex []) export "M" {acc: acc}] end M.acc push 1 end size
	// M.acc]` answers `[[1 1] 1 [1 1] 2]` for the interpreter's
	// `[[1] 1 [1] 1]` on main with no callback at all.
	`for 2 [import module [def acc (flex []) def stp fn [[e:Integer][Any][acc push e]] export "M" {stp: stp/v acc: acc}] end for-each M.stp [1 2 3] end size M.acc]`: {
		"NUR205 — an inline module import inside a loop body runs its module body once compiled, per iteration interpreted",
		"value divergence: compiled [3 6] vs interp [3 3]"},
	`for 2 [import module [def acc (flex []) def h fn [[m:Any][Any][acc push m.path]] export "M" {h: h/v acc: acc}] end walk {mode: "depth"} {a:1 b:[2 3]} M.h end size M.acc]`: {
		"NUR205 — an inline module import inside a loop body runs its module body once compiled, per iteration interpreted",
		"value divergence: compiled [{a:1 b:[2 3]} 5 {a:1 b:[2 3]} 10] vs interp [{a:1 b:[2 3]} 5 {a:1 b:[2 3]} 5]"},
