...
var sweepKnownMiscompiles = map[string]sweepPin{
	...
	// emit lead) was RETIRED 2026-09-26 (main's #510, the sweep's last
	// cells): the compile pass records the module's runtime fn-dispatch
	// native over a GRADUAL lead, which takes the interpreter's route for
	// whatever the value turns out to be, so the seed compiles with parity.
